...
class SubsBlocks extends CoreBase
{
	//Init Blocks
					...
					if ($active == "checked" && ($viewblock === true || $user_info['is_admin'])) {
						switch ($row['personal']) {
							...
							case '1':
								...
								$this->head($icon, $title, $id_block, $row['bk_collapse'], $row['bk_no_title'], $row['bk_style']);
								echo $content;
								$this->footer(trim($row['bk_style']));
								break;
								// PHP Block								
							case '2':
								$title = $user_info['is_admin'] ? '<a href="' . $scripturl . '?action=admin;area=ultimate_portal_blocks;sa=blocks-edit;id=' . $row['id'] . ';personal=' . $row['personal'] . ';type-php=created">' . $title . '</a>' : $title;
								$this->head($icon, $title, $id_block, $row['bk_collapse'], $row['bk_no_title'], $row['bk_style']);
								...
								// System Block			
							default:
								$title = $user_info['is_admin'] ? '<a href="' . $scripturl . '?action=admin;area=ultimate_portal_blocks;sa=blocks-edit;id=' . $row['id'] . ';personal=' . $row['personal'] . ';type-php=system">' . $title . '</a>' : $title;
								$this->head($icon, $title, $id_block, $row['bk_collapse'], $row['bk_no_title'], $row['bk_style']);
	...
	function head($icon, $title, $id_block = 0, $bk_collapse = 'on', $bk_no_title = '', $bk_style = 'on')
	{
	...
	function PrintPage(bool $left, bool $right, string $content = '', string $copyright = "", string $icon = '', string $title = '')
	{
		//Portal Above
		$this->printPageAbove($left, $right, $copyright, 0);
		$this->head($icon, $title, 0);
		//Ok now print
		echo $content;
		$this->footer();
		//Copyright?
	...
	function printPageAbove($left, $right, $copyright = '', $call_forum = 0, $call_front_page = 0)
	{
		...
		if (!empty($ultimateportalSettings['user_posts_header_show'])) {
			//Load Covers	
			$this->upCaller->subs()->getUserPostCoverShow();
			//Now control if your theme, it's "Curve" or "Variation Curve"
			echo '
	...
	function printPageBelow(bool $printRightColumn = false)
	{
		global $context, $options;
		global $ultimateportalSettings, $user_info;
		if (isset($_REQUEST['action']) && in_array($_REQUEST['action'], ['admin'])) return;

		echo '	
			</td>'; //CLose Column Center (Boards)

		//Column Right
		if ($printRightColumn && !empty($context['load_block_right'])) {
			echo '
	...
	function printMultiblock($position)
	{
		...
		if ($position == 'header') {
			if (!empty($ultimateportalSettings['up_reduce_site_overload'])) {
				if ((cache_get_data('mbk_header', 1800)) === NULL) {
					$this->upCaller->subs()->getBlockHeaderPortal("and enable=1");
					//Ultimate Portal use SMF Cache data... UP it's the best, only "UP", can create this feature
		...
		if ($position == 'footer') {
			if (!empty($ultimateportalSettings['up_reduce_site_overload'])) {
				if ((cache_get_data('mbk_footer', 1800)) === NULL) {
					$this->upCaller->subs()->getBlockFooterPortal("and enable=1");
					//Ultimate Portal use SMF Cache data... UP it's the best, only "UP", can create this feature
		...
		<td>';
			foreach ($context['block-' . $position] as $multiblock) {
				...
				$this->head('up-multiblock', $title, '-' . $multiblock['id'], $multiblock['mbk_collapse'], $multiblock['mbk_title'], $multiblock['mbk_style']);
				...
				<table id="up_bk_table_main" style="width:100%" cellpadding="5" cellspacing="2">
					...
					if (!empty($multiblock['vblocks']['c1'])) {
						...
						foreach ($multiblock['vblocks']['c1'] as $vblocks) {
							echo '
								', $this->getMultiBlockColumn($vblocks['id']), '';
						}
						echo '
							</td>';
					}
				...
				$this->footer($multiblock['mbk_style']);
			}
			echo '
		</td>
		</tr>
	...
	//Get Column
	function getMultiBlockColumn($id)
	{
